...
% solve (using the user-specified ODE solver in Solver_parameters.m)
% the time-varying linear ODEs defining the dynamics of
...
%                   1 Euler method on Jacobian + ODE solve on sensitivity ODE
%                   2 ODE solve on sensitivity ODE using explicit Jacobian function (requires Input_files/UP_Jacobian_Function.m)
...
% Extract from Solver_parameters.m the ODE solver to use
run('Solver_parameters.m');
ode_solver = parameters.ode_solver;
ode_options = parameters.ode_options;
% and the parameters for Euler approximations
...
euler_max_iter = parameters.euler_max_iter;       % Max number of iterations (each iteration divides the step by 2)
...
switch solver_choice
    case 1
        ...
        ode_solution = ode_solver(@(t, x) System_description(t, x, p), [t_init, t_final], x0, ode_options);
 
        % Solve state sensitivity equation dS_x(t)/dt=J_x(t)*S_x(t)
        if needed_sensitivies ~= 2
            [~, S_x_traj] = ode_solver(@(t, S_x) Sensitivity_system_state(t, S_x, ode_solution, p, euler_epsilon, euler_tolerance, euler_max_iter),...
                [t_init, t_final], reshape(eye(n_x), n_x*n_x, 1), ode_options);
            S_x = reshape(S_x_traj(end,:),n_x,n_x);
        end

        % Solve input sensitivity equation dS_p(t)/dt=J_x(t)*S_p(t)+J_p(t)
        if needed_sensitivies ~= 1
            [~, S_p_traj] = ode_solver(@(t, S_p) Sensitivity_system_input(t, S_p, ode_solution, p, euler_epsilon, euler_tolerance, euler_max_iter),...
                [t_init, t_final], zeros(n_x*n_p, 1), ode_options);
    ...
    case 2
        ...
        % Solve state sensitivity equation dS_x(t)/dt=J_x(t)*S_x(t)
        if needed_sensitivies ~= 2
            [~,S_x_traj] = ode_solver(@(t, S_x) Sensitivity_system_state(t, S_x, ode_solution, p, Handle_Jacobians),...
                [t_init, t_final], reshape(eye(n_x), n_x*n_x, 1), ode_options);
            S_x = reshape(S_x_traj(end,:),n_x,n_x);
        end
        ...
        % Solve input sensitivity equation dS_p(t)/dt=J_x(t)*S_p(t)+J_p(t)
        if needed_sensitivies ~= 1
            [~,S_p_traj] = ode_solver(@(t, S_p) Sensitivity_system_input(t, S_p, ode_solution, p, Handle_Jacobians),...
                [t_init, t_final], zeros(n_x*n_p, 1), ode_options);
            S_p = reshape(S_p_traj(end,:),n_x,n_p);
        end
    ...
    otherwise
        ...
        if needed_sensitivies ~= 2
            ...
            function_handle = @(x_arg) deval(ode_solver(@(t,x) System_description(t,x,p),[t_init,t_final],x_arg, ode_options),t_final);
        ...
        if needed_sensitivies ~= 1
            ...
            function_handle = @(p_arg) deval(ode_solver(@(t,x) System_description(t,x,p_arg),[t_init,t_final],x0, ode_options),t_final);
...
switch length(varargin)
    ...
    otherwise
        log.error('Wrong number of input arguments provided to function Sensitivity_system_state')
end
...
switch length(varargin)
    ...
    otherwise
        log.error('Wrong number of input arguments provided to function Sensitivity_system_input')
end
