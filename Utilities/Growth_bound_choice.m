...
%   growth_bound_method: integer to pick the method to obtain the growth bound
...
if isnan(growth_bound_method) || growth_bound_method == 1
    ...
    if ~any(isnan(GB_handle(1,ones(n_x,1),ones(n_p,1))))
        ...
        log.info('(2-1) Extract growth bound function handle from Input_files/UP_Growth_Bound_Function.m\n')
        return
    end
    
    % If undefined and user asked for this method
...
if n_p~=n_x || ...
   any(abs((System_description(t_final,x_low,p_up)-System_description(t_final,x_low,zeros(n_p,1))) - p_up) > eps(2) | ...
    ...
    if isnan(growth_bound_method)
        ...
        log.info('No growth bound function provided and dynamics do not have additive input.\n')
        bool_skip_CTGB = 1;
        GB_handle = NaN;
        return
    else
...
if isnan(growth_bound_method) || growth_bound_method == 2
    ...
    if ~any(isnan(C(:))) && (isequal(size(C),[n_x n_x]) || isscalar(C))
        log.info('(2-2) Extract growth bound function handle from contraction matrix in Input_files/UP_Contraction_Matrix.m\n')
...
if isnan(growth_bound_method) || growth_bound_method == 3
    ...
    if ~any(isnan(J_x_low(:))) && ~any(isnan(J_x_up(:)))
        log.info('(2-3) Extract growth bound function handle from state Jacobian bounds in Input_files/UP_Jacobian_Bounds.m\n')
...
% raise a flag that the growth bound method cannot be used
log.info('The requirements for the contraction/growth-bound method are not satisfied\n');
bool_skip_CTGB = 2;
GB_handle = NaN;
