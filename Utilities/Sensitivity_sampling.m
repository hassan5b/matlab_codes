...
%   sensitivity_sampling_pairs: (optional) number of pairs in [x_low,x_up]*[p_low,p_up] for which sensitivities are evaluated
...
function [S_x_low,S_x_up,S_p_low,S_p_up] = Sensitivity_sampling(t_init,t_final,x_low,x_up,p_low,p_up,solver_choice,sensitivity_sampling_pairs)
...
if nargin <= 7
    % Use value of 'sensitivity_sampling_pairs' from Solver_parameters.m if provided 
    run('Solver_parameters.m')
    sensitivity_sampling_pairs = parameters.sensitivity_sampling_pairs;
    
    % Otherwise, set it to sensitivity_sampling_per_dimension^(n_x+n_p)
    if isnan(sensitivity_sampling_pairs)
        sensitivity_sampling_per_dimension = parameters.sensitivity_sampling_per_dimension;
        
        % Ignore n_x or n_p if the corresponding interval is a singleton
        sensitivity_sampling_pairs = 1;
        if ~isequal(x_low,x_up)
            sensitivity_sampling_pairs = sensitivity_sampling_pairs*sensitivity_sampling_per_dimension^n_x;
        end
        if ~isequal(p_low,p_up)
            sensitivity_sampling_pairs = sensitivity_sampling_pairs*sensitivity_sampling_per_dimension^n_p;
        end
    end
end
...
for i = 1:sensitivity_sampling_pairs
...
toc_sampling = toc(tic_sampling);
log.runtime('Approximation of the sensitivity bounds based on %d state-input sample pairs took %f seconds\n', sensitivity_sampling_pairs, toc_sampling);
