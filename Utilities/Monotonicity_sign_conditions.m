%% Check if the provided system with sign-stable Jacobian matrices satisfies the continuous-time monotonicity property
...
if ~isequal(A,mod(A,2)) || ~isequal(b,mod(b,2))
    log.error('Non-binary elements provided.')
end

% Check if b has the correct dimension
if size(b,2)>1 || size(b,1)~=dim1
    log.error('Inconsistent dimensions of provided matrix and vector.')
end
...
if isempty(sol) || ~isequal(mod(A_init*sol,2),b_init)
    log.error('There was a GF(2) solver error.')
end
end
