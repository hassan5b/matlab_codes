...
if falsification_max_iter == 0
    ...
    log.info('Falsification of the sensitivity bounds skipped (falsification_max_iter = %d)',falsification_max_iter)
    return
end

% Skip falsification if the Optimization Toolbox is unavailable
if ~license('test','Optimization_Toolbox')
    log.warning('Falsification of the sensitivitiy bounds requires the Matlab Optimization Toolbox.\n')
    log.warning('Falsification skipped due to unavailable toolbox.\n')
    return
end
...
toc_falsification = toc(tic_falsification);
log.runtime('Falsification of the sensitivity bounds (no improvement after %d iterations) took %f seconds\n', iter, toc_falsification);
