...
switch system_choice
    ...
    case 13
        ...
        % Example section 8.1 of Interval Reachability Analysis
        % Circuit oscillates with a stable limit cycle.
        % Code from Appendix B.2. 
        ...
        % State Jacobian bounds
        J_x_low = [-inf,   1/C;...
                   -1/L,   -R/L];
        J_x_up = [.0074/C, 1/C;...
                  -1/L,    -R/L];
              
        % Input Jacobian bounds
        J_p_low = zeros(2);
        J_p_up = zeros(2);
    case 99
        % State x = [x; y; theta; v]
        th_lo = x_low(3);  th_hi = x_up(3);
        v_lo  = x_low(4);  v_hi  = x_up(4);
        v_max = max(abs([v_lo, v_hi]));

        % Conservative trig bounds over [th_lo, th_hi]
        % (safe and simple: |sin|,|cos| ≤ 1)
        sin_min = -1; sin_max = 1;
        cos_min = -1; cos_max = 1;

        % Initialize zeros
        J_x_low = zeros(4);  J_x_up = zeros(4);

        % Dynamics:
        % xdot = v*cos(theta) + d1  -> ∂xdot/∂theta = -v*sin(theta), ∂xdot/∂v = cos(theta)
        J_x_low(1,3) = -v_max;  J_x_up(1,3) =  v_max;      % -v*sin(theta)
        J_x_low(1,4) =  cos_min; J_x_up(1,4) =  cos_max;   % cos(theta)

        % ydot = v*sin(theta) + d2  -> ∂ydot/∂theta = v*cos(theta), ∂ydot/∂v = sin(theta)
        J_x_low(2,3) = -v_max;  J_x_up(2,3) =  v_max;      %  v*cos(theta)
        J_x_low(2,4) =  sin_min; J_x_up(2,4) =  sin_max;   % sin(theta)

        % thetadot = u1  (no x-dependence)
        % vdot    = u2   (no x-dependence)

        % Jacobian wrt inputs p = [u1; u2; d1; d2] (additive constants)
        J_p_low = zeros(4,4);  J_p_up = zeros(4,4);
        % ∂xdot/∂d1 = 1, ∂ydot/∂d2 = 1, ∂thetadot/∂u1 = 1, ∂vdot/∂u2 = 1
        J_p_low(1,3) = 1;  J_p_up(1,3) = 1;   % d1 -> xdot
        J_p_low(2,4) = 1;  J_p_up(2,4) = 1;   % d2 -> ydot
        J_p_low(3,1) = 1;  J_p_up(3,1) = 1;   % u1 -> thetadot
        J_p_low(4,2) = 1;  J_p_up(4,2) = 1;   % u2 -> vdot
