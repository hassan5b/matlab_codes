...
% methods several times with different parameter values in files
%   Examples/MAIN_CALL.m
%   Examples/MAIN_CALL_OA_comparison.m
...
parameters.OA_method = NaN;
    ...
    % Number of sample points for each state dimension
    %   (needs to be a vector of same length as the state variable)
    parameters.QMC_samples_per_state_dimension = 10 * ones(n_x, 1);
