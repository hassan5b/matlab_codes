...
%   1: Take growth bound from user-provided file
%       requires Input_files/UP_Growth_Bound_Function.m
%   2: Create a growth bound function from user-provided contraction matrix
%       requires Input_files/UP_Contraction_Matrix.m
%   3: Create a growth bound function from user-provided Jacobian bounds
...
% Successor of the system starting in init_center with input_center
[~, x_traj] = ode_solver(@(t,x) System_description(t, x, input_center), [t_init, t_final], state_center, ode_options);
OA_center = x_traj(end,:)';

% Size of the over-approximation due to the state and input variations,
%   using the provided growth bound function
