...
for i = 1:n_x
    ...
    [~, xi_low_traj] = ode_solver(@(t, x) System_description(t, x, pi_low(i,:)'),...
        [t_init, t_final], xi_low(i, :)', ode_options);
    % Trajectory from initial state xi_up(i,:)' with input pi_up(i,:)'
    [~, xi_up_traj] = ode_solver(@(t, x) System_description(t, x, pi_up(i, :)'),...
        [t_init, t_final], xi_up(i, :)', ode_options);
