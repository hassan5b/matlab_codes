...
function out = tv_cbf_varT_run(seed, runopts)

if nargin < 2 || isempty(runopts), runopts = struct(); end
if ~isfield(runopts,'doViz'),   runopts.doViz   = true;  end
if ~isfield(runopts,'doPlots'), runopts.doPlots = true;  end


rng(seed);   % reproducible randomness per run
...
c_log(:,1) = obs.c_fun(0);
D_log(1)   = obs.D_fun(0);
h_log(1)   = (X(1,1)-c_log(1,1))^2 + (X(2,1)-c_log(2,1))^2 - D_log(1)^2;
k_end = N;
...
for k = 1:N
    ...
    t_qp = tic;
    [u,~,flag] = quadprog(H,f,Aqp,bqp,[],[],[],[],[],qp_opts);
    solve_time_k = toc(t_qp);
    qp_flag_log(k) = flag;
    qp_time_log(k) = solve_time_k;
    ...
    % log obstacle and barrier
    c_log(:,k+1) = c;
    D_log(k+1)   = D;
    h_log(k+1)   = h_curr;

    comp_ms = 1000*toc(t_cycle);
    comp_ms_log(k) = comp_ms;
    ...
    if norm(X(1:2,k+1)-goal) < 0.5
        k_end = k;     % store where the run ended
        break;
    end
...
out.U_perf      = U_perf_log;

out.T_log       = T_log;
out.Tnext_log   = Tnext_log;
out.thist       = thist;

out.Mk_log      = Mk_log;
out.g_now_log   = g_now_log;

out.qp_flag_log = qp_flag_log;
out.qp_time_log = qp_time_log;
out.comp_ms_log = comp_ms_log;

out.c_log       = c_log;
out.D_log       = D_log;
out.h_log       = h_log;
out.h_min       = min(h_log(1:k_end+1), [], 'omitnan');

out.goal        = goal;
out.params = struct( ...
    'T0',0.1,'T_mpc',T_mpc,'N',N, ...
    'beta',beta,'Tmin',Tmin,'Tmax_hw',Tmax_hw,'deltaRT',deltaRT, ...
    'g_release',g_release,'grow_max',grow_max, ...
    'gamma',gamma,'epsM',epsM,'u_min',u_min,'u_max',u_max );
end
...
% In this merged script, the caller always uses T_mpc = 0.1.
    ...
    mpc_opts = optimoptions('fmincon','Display','off','MaxIterations',100,'Algorithm','sqp');
    Uopt = fmincon(@cost, U0, [],[],[],[], lb, ub, [], mpc_opts);
    u0   = Uopt(1:2);
end
